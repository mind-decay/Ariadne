...
<context>
tier-05 gave the daemon a socket and a `Ping`. This tier makes it useful: on startup it builds the warm `GraphIndex` (`ariadne-graph::build_from_snapshot`) and answers real queries over IPC (plan RD6). The watcher loop (tier-07) and the mcp/cli clients (tier-08/09) build on this protocol. Full context: plan.md.
</context>
...
- docs/adr/0014-daemon-mode-ipc.md — modify: resolve the deferred IPC-topology question.
</files>
...
3. Resolve the topology question in ADR-0014: protocol types stay in `ariadne-core`; transport stays in `ariadne-daemon`; clients (tier-08/09/15) embed a thin `daemon_client` module. If client duplication later exceeds one file, ADR-0014 may introduce a shared `ariadne-ipc` crate with an explicit `tests/architecture.rs` exception (precedent ADR-0007). Record the chosen shape now.
...
<rollback>
`git checkout -- crates docs/adr/0014-daemon-mode-ipc.md`. tier-05 (`Ping` skeleton) remains usable.
</rollback>
