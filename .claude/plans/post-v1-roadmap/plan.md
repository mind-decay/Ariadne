...
tiers:
  ...
  - tier-04-dead-code-classification
  - tier-05-daemon-skeleton
  - tier-06-daemon-warm-graph
  - tier-07-daemon-watcher-live
  - tier-08-mcp-daemon-client
  - tier-09-cli-daemon-client-slo
  - tier-10-git-history-ingest
  - tier-11-cyclomatic-complexity
  - tier-12-hotspot-cochange-metrics
  - tier-13-diff-aware-blast-radius
  - tier-14-analytics-mcp-tools
  - tier-15-lsp-skeleton
  - tier-16-lsp-navigation
  - tier-17-lsp-hierarchy
  - tier-18-hierarchy-impl-mcp-tools
---
...
In scope: native Go SCIP, redb schema migration, Astro semantic indexing, per-language dead-code classification; daemon + warm in-RAM graph; git-history hotspots/churn/co-change, cyclomatic complexity, diff-aware blast radius; LSP server adapter + call/type-hierarchy + implementations.
...
IPC topology: request/response wire types live in `ariadne-core/domain` (pure, no IO). The `interprocess` transport lives in `ariadne-daemon` (server side); driving adapters embed a thin `daemon_client` module. tier-05 ADR-0014 fixes the final topology — if per-adapter duplication exceeds one file, ADR-0014 introduces a shared `ariadne-ipc` crate with an explicit `tests/architecture.rs` exception (precedent: ADR-0007 carved out the composition root).
...
| redb | 4.1.0 (v1 pin) | schema-migration framework | 02 | https://docs.rs/redb/4.1.0/redb/struct.WriteTransaction.html |
| interprocess | 2.4.2 | daemon IPC (local socket) | 05 | https://docs.rs/interprocess/2.4.2/interprocess/ |
| gix | 0.83.0 | git history + diff (pure-Rust) | 10 | https://lib.rs/crates/gix |
| tree-sitter | 0.26.x (v1 pin) | cyclomatic complexity from CST | 11 | ariadne-core plan.md tech_inventory |
| async-lsp | 0.2.4 | LSP server adapter | 15 | https://lib.rs/crates/async-lsp |
</tech_inventory>
...
| R-B1 | IPC crate topology collides with the adapter-isolation invariant | medium | ADR-0014 fixes topology; ADR-0007 precedent permits a justified `tests/architecture.rs` exception |
