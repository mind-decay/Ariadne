...
<context>
v1 spawns the MCP server per Claude session; each tool cold-reads redb and rebuilds the graph. With the warm daemon (tier-07) the MCP server becomes a thin client: it forwards queries over IPC to the always-warm graph (plan RD6). The v1 cold path is retained as a fallback so a missing daemon never breaks Claude. Full context: plan.md.
</context>

<files>
- crates/ariadne-mcp/src/adapters/daemon_client.rs — new: thin `interprocess` client + framing (the per-adapter client module from ADR-0015).
...
2. Implement `daemon_client.rs`: connect to the `.ariadne/` local socket, length-prefix frame a `DaemonRequest`, read the `DaemonResponse` [src: https://docs.rs/interprocess/2.4.2/interprocess/local_socket/index.html ; ADR-0015].
...
4. Send the client's last-known redb `revision` in the handshake so the daemon refreshes if the client is ahead of the daemon's build (tier-07 handshake).
