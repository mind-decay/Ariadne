...
deps: [tier-06]
exit_criteria:
  - "A new `read_symbol` tool is registered on `AriadneServer` and appears in the rmcp tool list."
...
slice [src: plan.md D9; catalog.rs:77-79 root; storage tables.rs:15-36]. It is the
...
2. **Types.** `ReadSymbolInput { symbol: String, file: Option<String> (disambiguate
   ...
   `Deserialize, Serialize, JsonSchema` set [src: types.rs:35-50,69-80].
...
4. **Handler.** Resolve `symbol` via `cat.by_name` (use `file` to disambiguate when
   several match; else first + note), fetch `SymbolMeta` span + path, call
   `read_span`, attach `cat.revision` [src: catalog.rs:69-72,129-145].
5. **Register.** Add the `#[tool]` method on `AriadneServer` with a trigger-phrase
   ...
   one-line [src: server.rs:184-205].
</steps>
