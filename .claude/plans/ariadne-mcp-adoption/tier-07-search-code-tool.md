...
deps: [tier-06]
exit_criteria:
  - "A new `search_code` tool is registered on `AriadneServer` and appears in the rmcp tool list alongside the existing 13."
...
port [src: plan.md D8; catalog.rs:60-153; tools/list_symbols.rs:11-32]. `regex`
...
- `crates/ariadne-mcp/src/types.rs` — `SearchCodeInput` (mirrors `ListSymbolsInput`
  derive set + `#[serde(default)]` optionals) [src: types.rs:69-80].
...
- `crates/ariadne-mcp/src/server.rs` — `#[tool]` method delegating one-line, like
  `list_symbols` [src: server.rs:184-205].
...
4. **Handler.** Compile the name matcher once: substring (lowercase `contains`,
   ...
   early-exit at `limit`, map via `summarize` [src: tools/summarize mod.rs:29-59].
...
6. **Register.** Add the `#[tool(...)]` method on `AriadneServer` with a
   ...
   kind"), delegating to `tools::search_code::handle` [src: server.rs:184-205].
7. **Bench.** Criterion case over a synthetic ~100k-symbol catalog asserting median
