...
- Use here: `find_references` + `blast_radius`/`diff_blast_radius` before and after editing a symbol to find call sites and scope impact, `affected_tests` to find which tests the edit reaches before running `<verification>` (steps 3, 5); `plan_assist` to scope which files a change touches; `search_code`/`read_symbol`/`read_outline`/`file_summary`/`find_definition`/`list_symbols` to navigate and read instead of `grep` (`read_outline` folds a whole file to a skeleton) [src: CLAUDE.md; ariadne MCP tool catalog].
</code_intelligence>
