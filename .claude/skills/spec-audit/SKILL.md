...
- Use here: `coupling_report`/`weak_spots`/`refactor_suggestions` to verify architecture matches `<decisions>`; `diff_blast_radius`/`blast_radius`/`find_references` to confirm nothing outside `<files>` is affected and no dangling refs (steps 2, 3); `hotspots`/`complexity`/`co_change` to triage risk in the changed code; `search_code`/`read_symbol`/`file_summary`/`find_definition`/`list_symbols` to read the diff. Augments the evidence pass — never replaces reading the diff [src: CLAUDE.md].
</code_intelligence>
