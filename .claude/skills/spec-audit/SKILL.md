...
- Use here: `coupling_report`/`weak_spots`/`refactor_suggestions` plus `fitness_report` to verify architecture matches `<decisions>` and the project's `ariadne-fitness.toml` rules still hold; `diff_blast_radius`/`blast_radius`/`find_references` to confirm nothing outside `<files>` is affected and no dangling refs, `affected_tests` to confirm the diff's reachable tests were run, `api_surface_diff` to catch unintended breaking public-API changes (steps 2, 3); `hotspots`/`complexity`/`co_change` to triage risk in the changed code; `search_code`/`read_symbol`/`read_outline`/`file_summary`/`find_definition`/`list_symbols` to read the diff. Augments the evidence pass — never replaces reading the diff [src: CLAUDE.md; ariadne MCP tool catalog].
</code_intelligence>
