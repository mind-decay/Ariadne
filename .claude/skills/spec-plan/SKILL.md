...
- Use here: `plan_assist` + `blast_radius` to scope impact and integration boundaries (step 2); `coupling_report`/`weak_spots`/`refactor_suggestions` to ground architecture decisions; `find_references`/`list_symbols`/`find_definition` to map touched code (steps 2, 4) [src: CLAUDE.md].
</code_intelligence>
