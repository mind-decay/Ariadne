...
- Use here: `plan_assist` + `blast_radius`/`diff_blast_radius` to scope impact and integration boundaries (step 2); `coupling_report`/`weak_spots`/`refactor_suggestions` plus `hotspots`/`complexity`/`co_change` to ground architecture decisions in structure and Git risk; `search_code`/`read_symbol`/`file_summary`/`find_references`/`list_symbols`/`find_definition` to map touched code; `doc_for`/`doc_for_module`/`doc_for_project` to summarize the area being changed (steps 2, 4) [src: CLAUDE.md].
</code_intelligence>
