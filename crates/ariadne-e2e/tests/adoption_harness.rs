...
fn count_tool_uses(message: &Value, tally: &mut Tally) {
    ...
    for block in content {
        ...
        match block.get("name").and_then(Value::as_str) {
            Some(name) if name.starts_with("mcp__ariadne__") => tally.ariadne += 1,
...
fn report_tally(label: &str, t: &Tally) {
    ...
    println!(
        "[adoption] {label} | ariadne={} grep={} read={} glob={} \
         | ratio(ariadne:grep+read)={ratio} ariadne_share={share} \
         | tokens in={} out={}",
        t.ariadne, t.grep, t.read, t.glob, t.input_tokens, t.output_tokens,
    );
}
