//! Tier-03 step 10: Vue SFC incremental-parse equivalence proptest.
//!
//! 100 random `InputEdit` sequences are applied to a `.vue` fixture. After
...
//! divergence fails loud: the injection refactor must not break incremental
//! correctness (plan.md R-Inject; tier-03 exit criterion 4).
...
const SAMPLE: &str = include_str!("../fixtures/vue/sample.vue");
...
proptest! {
    ...
    #[test]
    fn incremental_vue_matches_full_reparse(specs in arb_edit_seq()) {
        let registry = ParserRegistry::new();
        let mut content = SAMPLE.as_bytes().to_vec();
        let mut parsed = parse_file(Lang::Vue, &registry, &content, None, &[])
            .expect("cold parse");
        for spec in specs {
            let (new_content, input_edit) = apply_edit(&content, &spec);
            let inc = parse_file(Lang::Vue, &registry, &new_content, Some(&parsed), &[input_edit])
                .expect("incremental parse");
            let full = parse_file(Lang::Vue, &registry, &new_content, None, &[])
                .expect("full parse");
