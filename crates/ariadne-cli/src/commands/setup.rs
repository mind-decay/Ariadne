...
/// or defers. It classifies two shapes: a symbol-shaped Grep/Glob pattern (which
/// `search_code` + the nav tools answer) and a whole-file Read of a source file
/// (which `read_symbol` answers). Advisory by construction: it emits only `allow`
