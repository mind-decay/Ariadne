...
/// `jq`, no `ariadne`) that reads the `PreToolUse` payload on stdin and either
/// injects advisory context (`permissionDecision:"allow"` + `additionalContext`)
/// or defers. It classifies these shapes: a symbol-shaped Grep/Glob pattern (which
/// `search_code` + the nav tools answer); a whole-file source Read with no symbol
/// target (which `read_outline` folds to a skeleton); and a ranged source Read
/// carrying an offset/limit (which `read_symbol` answers). Advisory by construction: it emits only `allow`
