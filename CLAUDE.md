...
- Commit format: Conventional Commits v1.0.0 `<type>(<scope>)<!>: <subject>` [src: https://www.conventionalcommits.org/en/v1.0.0/]. Types: feat, fix, docs, style, refactor, perf, test, build, ci, chore, revert. Scopes = crate names without `ariadne-` prefix (core, storage, parser, scip, graph, salsa, watcher, mcp, cli, e2e) + cross-cutting (docs, ci, deps). Subject ≤72 chars, imperative. Breaking change: `!` after scope OR `BREAKING CHANGE:` footer. Enforced by lefthook commit-msg hook + CI `cog check` + PR-title action [src: .claude/plans/ariadne-core/plan.md D14, https://github.com/cocogitto/cocogitto].
